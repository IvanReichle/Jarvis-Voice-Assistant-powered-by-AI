...
def _calibrar_umbral(stream, chunk_samples, muestras=8):
    ...
    return max(float(np.mean(niveles)) * 3.0, 0.008)
...
class JarvisApp:
    ...
    def _setup_ui(self):
        ...
        # Overlay suave solo en la zona del log
        log_top = int(SH * 0.42)
        ...
        # Titulo (guardamos referencia para pulsarlo)
        self._title_id = self.canvas.create_text(
            SW // 2, int(SH * 0.06),
            text="J.A.R.V.I.S",
            font=("Segoe UI", 32, "bold"), fill="#00b4d8", anchor="center"
        )
        self.canvas.create_line(
            int(SW * 0.35), int(SH * 0.11),
            int(SW * 0.65), int(SH * 0.11),
            fill="#00b4d8", width=1
        )

        # Brackets de esquina (estaticos, sobre el canvas)
    ...
    def _draw_corner_brackets(self):
        ...
        for x1, y1, x2, y2 in coords:
            self.canvas.create_line(x1, y1, x2, y2, fill=c, width=w, tags="hud_brackets")
    ...
    def _animate(self):
        ...
        cx = SW // 2
        cy = int(SH * 0.50)   # centro del circulo del fondo
        log_top = int(SH * 0.44)

        # ── Anillo exterior giratorio (lento, cian) ──
        self._a1 = (self._a1 + 0.25) % 360
        r1 = int(SH * 0.355)
        self.canvas.create_arc(
            cx-r1, cy-r1, cx+r1, cy+r1,
            start=self._a1, extent=140, style="arc",
            outline="#00b4d8", width=1, tags="anim"
        )
        self.canvas.create_arc(
            cx-r1, cy-r1, cx+r1, cy+r1,
            start=self._a1+190, extent=90, style="arc",
            outline="#1a5f7a", width=1, tags="anim"
        )
        # Marcas de tick en anillo exterior
        for i in range(24):
            a = math.radians(self._a1 + i * 15)
            r_out = r1
            r_in = r1 - (16 if i % 6 == 0 else 8 if i % 3 == 0 else 4)
            x1c = cx + r_out * math.cos(a)
            y1c = cy + r_out * math.sin(a)
            x2c = cx + r_in  * math.cos(a)
            y2c = cy + r_in  * math.sin(a)
            col = "#00b4d8" if i % 6 == 0 else "#1a4a5a" if i % 3 == 0 else "#0d2a35"
            lw  = 2 if i % 6 == 0 else 1
            self.canvas.create_line(x1c, y1c, x2c, y2c, fill=col, width=lw, tags="anim")

        # ── Anillo medio contra-rotatorio (rojo) ──
        self._a2 = (self._a2 - 0.55) % 360
        r2 = int(SH * 0.235)
        self.canvas.create_arc(
            cx-r2, cy-r2, cx+r2, cy+r2,
            start=self._a2, extent=110, style="arc",
            outline="#cc2a20", width=2, tags="anim"
        )
        self.canvas.create_arc(
            cx-r2, cy-r2, cx+r2, cy+r2,
            start=self._a2+170, extent=60, style="arc",
            outline="#ff5040", width=1, tags="anim"
        )
        # Puntos en el anillo medio
        for i in range(6):
            a = math.radians(self._a2 + i * 60)
            xd = cx + r2 * math.cos(a)
            yd = cy + r2 * math.sin(a)
            self.canvas.create_oval(xd-4, yd-4, xd+4, yd+4,
                                    fill="#ff3b30", outline="#ff8070", tags="anim")

        # ── Anillo interior (reactor, verde rapido) ──
        self._a3 = (self._a3 + 1.1) % 360
        r3 = int(SH * 0.095)
        for offset in (0, 120, 240):
            self.canvas.create_arc(
                cx-r3, cy-r3, cx+r3, cy+r3,
                start=self._a3+offset, extent=85, style="arc",
                outline="#00e676", width=2, tags="anim"
            )

        # ── Linea de escaneo horizontal ──
        scan_h = log_top - int(SH * 0.12)
        self._scan = (self._scan + 1.4) % scan_h
        sy = int(SH * 0.12) + int(self._scan)
        self.canvas.create_line(
            int(SW * 0.04), sy, int(SW * 0.96), sy,
            fill="#1a3a4a", width=3, tags="anim"
        )
        self.canvas.create_line(
            int(SW * 0.04), sy, int(SW * 0.96), sy,
        ...
        # ── Pulso en el titulo (color ciclico) ──
